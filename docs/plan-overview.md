...
- 跨端一致：核心体验当前在 `iOS` / `iPadOS` 保持一致目标和规则，后续再扩展其他平台。
...
### 3.1 决策项列表
- D1：技术栈选型策略（已决策：Apple-first native，`Swift 6 + SwiftUI + SwiftData`）。
- D2：跨端交付路径（已决策：当前 MVP 采用 Apple-first；`Android / Windows` deferred，后续按原生平台能力评估）。
- D3：数据存储与同步策略（已决策：local-first，后续 Apple 设备间同步优先 `CloudKit private database`）。
- D4：展示触点策略（已调整：当前版本聚焦 `iOS` / `iPadOS` 主流程与高保真原型，不包含 `macOS` 展示触点）。
- D5：验收标准定义（未开始：功能完成、体验完成、发布完成的判定口径）。
...
### 8.1 OpenSpec 变更
- 当前活跃变更：`openspec/changes/confirm-technical-stack/`
- 状态：技术栈确认任务已完成，后续可进入工程实现基线落地。
